...
# nelements is needed for the bounds check in the kernel
@triton.jit
def add_kernel(x, y, output, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    output_segment = x_segment + y_segment
    tl.store(output + offset_from_start, output_segment, mask=mask)

# 2- Let's first write a wrapper CPU function that'll call the GPU function
...
# 1- We create the numbers on the GPU here 
...
# Verify correctness
assert torch.allclose(output_triton, output_torch), "Triton addition does not match PyTorch addition!"
