...
int main(){
    ...
    //cudaDeviceSynchronize(); // you can synchronize here across all kernels on the device, not needed here
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop); // waits until the stop event is recorded 
    
    float milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("Kernel execution time: %f ms\n", milliseconds / 100);
    printf("Bandwidth: %f GB/s\n", (3.0 * size) / (milliseconds / 1000 / 100) / 1e9);
